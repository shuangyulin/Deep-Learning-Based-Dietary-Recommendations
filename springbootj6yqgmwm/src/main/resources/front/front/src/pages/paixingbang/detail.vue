	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/paixingbang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">更新日期</div>
						<div class="text "  >{{detail.gengxinriqi}}</div>
					</div>
					<div class="item">
						<div class="lable">第一名</div>
						<div class="text "  >{{detail.diyiming}}</div>
					</div>
					<div class="item">
						<div class="lable">使用次数</div>
						<div class="text "  >{{detail.shiyongcishu}}</div>
					</div>
					<div class="item">
						<div class="lable">链接</div>
						<div class="text  link"  @click="linkOthers(detail.lianjie)">{{detail.lianjie}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('paixingbang','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('paixingbang','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('paixingbang/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'paixingbang', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getDiscussList(page) {
				this.$http.get('discusspaixingbang/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusspaixingbang/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discusspaixingbang/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discusspaixingbang/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此排行榜？') .then(_ => {
