		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.pengrenshijian" placeholder="烹饪时间" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">食材：</div>
					<el-input v-model="formSearch.shicai" placeholder="食材" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">烹饪方式：</div>
					<el-input v-model="formSearch.pengrenfangshi" placeholder="烹饪方式" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('shiwuxinxi','新增')" type="primary" @click="add('/index/shiwuxinxiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.shiwutupian)" v-if="item.shiwutupian && item.shiwutupian.substr(0,4)=='http'&& item.shiwutupian.split(',w').length>1" :src="item.shiwutupian" />
						<img class="image" @click.stop="imgPreView(item.shiwutupian.split(',')[0])" v-else-if="item.shiwutupian && item.shiwutupian.substr(0,4)=='http'" :src="item.shiwutupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.shiwutupian?item.shiwutupian.split(',')[0]:''))" v-else :src="baseUrl + (item.shiwutupian?item.shiwutupian.split(',')[0]:'')" />
						<div class="name">{{item.shiwumingcheng}}</div>
						<div class="name">{{item.pengrenfangshi}}</div>
						<div class="name">能量:{{item.nengliang}}</div>
						<div class="name">蛋白质g:{{item.danbaizhig}}</div>
						<div class="name">脂肪g:{{item.zhifangg}}</div>
						<div class="time_item">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.pengrenshijian != '') searchWhere.pengrenshijian = '%' + this.formSearch.pengrenshijian + '%';
				if (this.formSearch.shicai != '') searchWhere.shicai = '%' + this.formSearch.shicai + '%';
				if (this.formSearch.pengrenfangshi != '') searchWhere.pengrenfangshi = '%' + this.formSearch.pengrenfangshi + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`shiwuxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/shiwuxinxiDetail', query: params});
			},
