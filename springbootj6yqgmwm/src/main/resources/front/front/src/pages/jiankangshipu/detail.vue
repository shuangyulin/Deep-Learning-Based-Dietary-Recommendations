	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiankangshipu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">食谱类型</div>
						<div class="text "  >{{detail.shipuleixing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">蛋白质</div>
						<div class="text "  >{{detail.danbaizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">脂肪</div>
						<div class="text "  >{{detail.zhifang}}</div>
					</div>
					<div class="item">
						<div class="lable">碳水化合物</div>
						<div class="text "  >{{detail.tanshuihuahewu}}</div>
					</div>
					<div class="item">
						<div class="lable">制作流程</div>
						<div class="text "  >{{detail.zhizuoliucheng}}</div>
					</div>
					<div class="item">
						<div class="lable">食材搭配</div>
						<div class="text "  >{{detail.shicaidapei}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiankangshipu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiankangshipu','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="营养信息" name="first">
					<div class="ql-snow ql-editor" v-html="detail.yingyangxinxi"></div>
				</el-tab-pane>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.shipufengmian) {
							this.detailBanner = this.detail.shipufengmian.split(",w").length>1?[this.detail.shipufengmian]:this.detail.shipufengmian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('jiankangshipu/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'jiankangshipu', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jiankangshipu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussjiankangshipu/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussjiankangshipu/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussjiankangshipu/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$http.get('discussjiankangshipu/page',{params: {
							page: 1,
							limit: 1,
							...
							this.$http.post('discussjiankangshipu/add', this.form).then(rs2 => {
								if (rs2.data.code == 0) {
			...
			addDiscussNum(type){
				...
				this.$http.get('discussjiankangshipu/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiankangshipuAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此健康食谱？') .then(_ => {
					this.$http.post('jiankangshipu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
