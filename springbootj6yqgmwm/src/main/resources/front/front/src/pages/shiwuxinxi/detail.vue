	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shiwuxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">烹饪时间</div>
						<div class="text "  >{{detail.pengrenshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">食材</div>
						<div class="text "  >{{detail.shicai}}</div>
					</div>
					<div class="item">
						<div class="lable">烹饪方式</div>
						<div class="text "  >{{detail.pengrenfangshi}}</div>
					</div>
					<div class="item">
						<div class="lable">能量</div>
						<div class="text "  >{{detail.nengliang}}</div>
					</div>
					<div class="item">
						<div class="lable">蛋白质g</div>
						<div class="text "  >{{detail.danbaizhig}}</div>
					</div>
					<div class="item">
						<div class="lable">脂肪g</div>
						<div class="text "  >{{detail.zhifangg}}</div>
					</div>
					<div class="item">
						<div class="lable">钙mg</div>
						<div class="text "  >{{detail.gaimg}}</div>
					</div>
					<div class="item">
						<div class="lable">铁mg</div>
						<div class="text "  >{{detail.tiemg}}</div>
					</div>
					<div class="item">
						<div class="lable">锌mg</div>
						<div class="text "  >{{detail.xinmg}}</div>
					</div>
					<div class="item">
						<div class="lable">维生素</div>
						<div class="text "  >{{detail.weishengsu}}</div>
					</div>
					<div class="item">
						<div class="lable">碳水化合物g</div>
						<div class="text "  >{{detail.tanshuihuahewug}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shiwuxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shiwuxinxi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.shiwutupian) {
							this.detailBanner = this.detail.shiwutupian.split(",w").length>1?[this.detail.shiwutupian]:this.detail.shiwutupian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('shiwuxinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'shiwuxinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'shiwuxinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'shiwuxinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shiwuxinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discussshiwuxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussshiwuxinxi/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussshiwuxinxi/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$http.get('discussshiwuxinxi/page',{params: {
							page: 1,
							limit: 1,
							...
							this.$http.post('discussshiwuxinxi/add', this.form).then(rs2 => {
								if (rs2.data.code == 0) {
			...
			addDiscussNum(type){
				...
				this.$http.get('discussshiwuxinxi/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id
				}}).then(rs=>{
			...
			// 修改
			editClick(){
				this.$router.push(`/index/shiwuxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此食物信息？') .then(_ => {
					this.$http.post('shiwuxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
