	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('yingyangjianyi','新增')" type="primary" @click="add('/index/yingyangjianyiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="name">体重kg:{{item.tizhongkg}}</div>
						<div class="name">身高m:{{item.shengaom}}</div>
						<div class="name">bmi:{{item.bmi}}</div>
						<div class="name">{{item.pinggushijian}}</div>
						<div class="name">健康程度:{{item.jiankangchengdu}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'健康程度',list:this.jiankangchengduOptions,tableName: 'jiankangchengdu',check: -1})
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.jiankangchengdu != '') searchWhere.jiankangchengdu = this.formSearch.jiankangchengdu;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`yingyangjianyi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yingyangjianyiDetail', query: params});
			},
