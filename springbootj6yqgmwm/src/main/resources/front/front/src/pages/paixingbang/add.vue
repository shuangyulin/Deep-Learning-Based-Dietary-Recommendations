<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="更新日期" prop="gengxinriqi">
				<el-date-picker
					:disabled=" false  ||ro.gengxinriqi"
			...
			<el-form-item class="add-item" label="第一名" prop="diyiming">
				<el-input v-model="ruleForm.diyiming" 
					placeholder="第一名" clearable :disabled=" false  ||ro.diyiming"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="使用次数" prop="shiyongcishu">
				<el-input v-model.number="ruleForm.shiyongcishu" 
					placeholder="使用次数" clearable :disabled=" false  ||ro.shiyongcishu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="链接" prop="lianjie">
...
<script>
	export default {
		data() {
			return {
				...
				rules: {
					...
					diyiming: [
						{ required: true, message: '第一名不能为空', trigger: 'blur' },
					],
					shiyongcishu: [
		...
		created() {
			...
			this.ruleForm.gengxinriqi = this.getCurDate()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='diyiming'){
							this.ruleForm.diyiming = obj[o];
							this.ro.diyiming = true;
							continue;
						}
						if(o=='shiyongcishu'){
							this.ruleForm.shiyongcishu = obj[o];
							this.ro.shiyongcishu = true;
							continue;
						}
						if(o=='lianjie'){
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`paixingbang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
