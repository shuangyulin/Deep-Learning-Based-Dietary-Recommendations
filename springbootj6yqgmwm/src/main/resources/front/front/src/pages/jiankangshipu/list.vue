	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('jiankangshipu','新增')" type="primary" @click="add('/index/jiankangshipuAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.shipufengmian)" v-if="item.shipufengmian && item.shipufengmian.substr(0,4)=='http'&& item.shipufengmian.split(',w').length>1" :src="item.shipufengmian" />
						<img class="image" @click.stop="imgPreView(item.shipufengmian.split(',')[0])" v-else-if="item.shipufengmian && item.shipufengmian.substr(0,4)=='http'" :src="item.shipufengmian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.shipufengmian?item.shipufengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.shipufengmian?item.shipufengmian.split(',')[0]:'')" />
						<div class="name">{{item.shipumingcheng}}</div>
						<div class="name">{{item.shipuleixing}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'食谱类型',list:this.shipuleixingOptions,tableName: 'shipuleixing',check: -1})
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.shipuleixing != '') searchWhere.shipuleixing = this.formSearch.shipuleixing;
				if (this.formSearch.xingming != '') searchWhere.xingming = '%' + this.formSearch.xingming + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiankangshipu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiankangshipuDetail', query: params});
			},
